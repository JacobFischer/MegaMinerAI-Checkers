...
static bool parseChecker(Checker& object, sexp_t* expression)
{
  sexp_t* sub;
  if ( !expression ) return false;
  sub = expression->list;
  ...
  if(expression->val != NULL && strcmp(expression->val, "status") == 0)
  {
    ...
    while(expression->next != NULL)
    {
      ...
      if(string(sub->val) == "game")
      {
          sub = sub->next;
          if ( !sub ) return false;
          gs.boardWidth = atoi(sub->val);
          sub = sub->next;
          if ( !sub ) return false;
          gs.boardHeight = atoi(sub->val);
          sub = sub->next;
          if ( !sub ) return false;
      ...
      else if(string(sub->val) == "Checker")
      {
        ...
        {
          Checker object;
          flag = parseChecker(object, sub);
          gs.checkers[object.id] = object;
          sub = sub->next;
        }
        if ( !flag ) return false;
      }
