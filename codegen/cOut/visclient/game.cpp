...
DLLEXPORT void destroyConnection(Connection* c)
{
  ...
  if(c->Checkers)
  {
    for(int i = 0; i < c->CheckerCount; i++)
    {
    }
    delete[] c->Checkers;
  }
...
void parsePlayer(Connection* c, _Player* object, sexp_t* expression)
{
  ...
  sub = sub->next;
  object->yDirection = atoi(sub->val);
  sub = sub->next;

}
void parseChecker(Connection* c, _Checker* object, sexp_t* expression)
{
  sexp_t* sub;
  sub = expression->list;
  ...
  while(true)
  {
    ...
    else if(expression->val != NULL && strcmp(expression->val, "status") == 0)
    {
      while(expression->next != NULL)
      {
        ...
        {
          sub = sub->next;
          c->boardWith = atoi(sub->val);
          sub = sub->next;

          c->boardHeight = atoi(sub->val);
          sub = sub->next;
        ...
        else if(string(sub->val) == "Checker")
        {
          if(c->Checkers)
          {
            for(int i = 0; i < c->CheckerCount; i++)
          ...
          c->CheckerCount =  sexp_list_length(expression)-1; //-1 for the header
          c->Checkers = new _Checker[c->CheckerCount];
          for(int i = 0; i < c->CheckerCount; i++)
          {
            sub = sub->next;
            parseChecker(c, c->Checkers+i, sub);
          }
        }
      }
      destroy_sexp(base);
...
{
  return c->Checkers + num;
}
DLLEXPORT int getCheckerCount(Connection* c)
{
  return c->CheckerCount;
}


DLLEXPORT int getBoardWith(Connection* c)
{
  return c->boardWith;
}
DLLEXPORT int getBoardHeight(Connection* c)
{
  return c->boardHeight;
}
